...
protocol LexisLookupable {
    associatedtype GrammaticalCategory
    subscript(_ key: String) -> [GrammaticalCategory] { get set }
}
struct CategoryLexicon {
    var entries: [String : [Category]]
}
extension CategoryLexicon : LexisLookupable {
    ...
    mutating func addEntry(word: String, category: Category) {
        if self[word].contains(category){
            return
        } else {
...
func parse(_ input: [String], withLexicon lexicon: CategoryLexicon) -> [ParseTree] {
